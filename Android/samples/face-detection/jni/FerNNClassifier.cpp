...
void FerNNClassifier::prepare(const vector<Size>& scales){
  ...
  //features的维数：scales.size() * totalFeatures（这里只指定了vector的长度，并没有赋初值）
  features = vector<vector<Feature> >(scales.size(),vector<Feature> (totalFeatures));
  //rng为随机生成数
  RNG& rng = theRNG();
  float x1f,x2f,y1f,y2f;
  int x1, x2, y1, y2;
  //集合分类器基于n个基本分类器，每个分类器都基于一个pixel comparisons（像素比较集）；  
  //pixel comparisons产生：先用一个归一化的patch去离散化像素空间，产生所有可能的垂直和水平的pixel comparisons  
  ...
  //注意这些二维向量的元素个数：2^StructSize * nStructs
...
//得到某一scale对应行的特征值，其中每个int元素代表一棵树的相应特征值
void FerNNClassifier::getFeatures(const cv::Mat& image,const int& scale_idx, vector<int>& fern){
  int leaf;//默认int为16（或者32位），StructSize = 13,可以用一个int来代替StructSize个bool值
