...
void update_hue_image(Mat mRgba, Rect[] region, CamShifting cs) {
	...
	//mask out-of-range values
	  Core.inRange(cs.obj.hsv, new Scalar(0, smin,Math.min(vmin,vmax)),new Scalar(180, 256,
			        Math.max(vmin, vmax)), cs.obj.mask);
...
RotatedRect camshift_track_face(Mat mRgba,Rect[] region,CamShifting cs)
	{
		...
		cs.obj.curr_box=Video.CamShift(cs.obj.prob, cs.obj.prev_rect, new TermCriteria(TermCriteria.EPS,5,1));	
