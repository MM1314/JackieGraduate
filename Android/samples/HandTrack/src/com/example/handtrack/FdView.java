...
class FdView extends SampleCvViewBase {
    ...
    private static final Scalar   FACE_RECT_COLOR1 = new Scalar(0, 255, 255, 255);
 ...
 //private CameraBridgeViewBase   mOpenCvCameraView;
    ...
    public FdView(Context context) {
        ...
        try {
          //  hp=new HeadPose();//imran
        	cs = new CamShifting();
        	InputStream is = context.getResources().openRawResource(R.raw.palm);
    ...
    protected Bitmap processFrame(VideoCapture capture) {
        ...
        if(!nextFrame)
        {
        	...
        	if (facearray1.length>0)
        	{
        	for (int i = 0; i < facearray1.length; i++)
                Core.rectangle(mRgba, facearray1[i].tl(), facearray1[i].br(), FACE_RECT_COLOR, 3);
        	
        	
        	cs.create_tracked_object(mRgba,facearray1,cs);
        	}
        	//nextFrame=true;
        ...
        if(nextFrame)
        {
        	...
        	Core.ellipse(mRgba,face_box,FACE_RECT_COLOR1, 6);	
        }
